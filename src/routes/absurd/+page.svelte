<script lang='ts'>
	import { createAbsurdMediaPrompts, createMediaPrompts, generateImage, generateLogicalChunksForImageContext, generateTranscript, type EnrichedLogicalChunk, type LogicalChunk } from "$lib/api/ai";
    ...
    let creatingAbsurdMediaPrompts = $state(false);
    let absurdEnrichedChunks = $state<EnrichedChunkWithImage[]>([]);
    async function handleGenerateAbsurdMediaPrompt(generatedTextChunks: LogicalChunk[]): Promise<void> {
        if (creatingAbsurdMediaPrompts || !generatedTextChunks) return;

        creatingAbsurdMediaPrompts = true;
        try {
            let response = await createAbsurdMediaPrompts(generatedTextChunks);
            ...
            creatingAbsurdMediaPrompts = false;
        } catch (error) {
            creatingAbsurdMediaPrompts = false;
    ...
    async function handleEntireFlow(uploadedFile: File | null = null): Promise<void> {
        ...
        if (files && files.length > 0) {
            ...
            await handleGenerateAbsurdMediaPrompt(generatedTextChunks);

            // await handleCreateMediaPrompt(generatedTextChunks);

            console.log("enrichedChunks:", enrichedChunks)
